...
<?php
	...
	public function process_field($field, $record, $ajax_handler)
	{
		...
		$form_data 			  = $record->get("form_settings");
		$uploads              = wp_upload_dir();
		$form_directory       = absint($form_data['id']) . '-' . md5($form_data['id']);
	...
	function add_lib()
	{
		...
		wp_enqueue_script(
			"elementor_signature_lib",
			SUPERADDONS_ELEMENTOR_SIGNATURE_PLUGIN_URL . "lib/js/jquery.signature.js",
			array('jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-mouse', 'jquery-touch-punch'),
			'1.0.0',
			true
		);
		wp_enqueue_script(
			"elementor_signature",
			SUPERADDONS_ELEMENTOR_SIGNATURE_PLUGIN_URL . "lib/js/signature.js",
			array("jquery", "elementor_signature_lib"),
			time(),
			true
		);
		wp_enqueue_style(
			"elementor_signature",
			SUPERADDONS_ELEMENTOR_SIGNATURE_PLUGIN_URL . "lib/css/jquery.signature.css",
			array(),
			'1.0.0'
		);
	}
